...
const MyDialog = ({ show, proceed, message }: ConfirmDialogProps<{ message: string }, boolean>) => (
    <Box style={{ display: show ? 'block' : 'none' }} sx={{ position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 9999 }}>
        <Box sx={{ margin: '20px auto', p: 5, borderRadius: 2, width: '100%', bgcolor: '#fff9f9ff', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2 }}>
            <p>{message}</p>
